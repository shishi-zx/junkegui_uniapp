<template>
    <div class="container">
        ...
        <div class="list-box">
            <uni-list class="list">
                <my-link :to="`{ path: '/pages/martyrCemetery/information', query: { id: ${item.id},type:1 }}`" navType="push"
...
<script>
import { reqAllMartyrCemetery,reqDelMartyrCemetery } from '../../api/index'
export default {
    data() {
        return {
            list: [],
    ...
    methods: {
        async init() {
            let res = await reqAllMartyrCemetery()
            if (res.code != 200) {
        ...
        async onDelete(id) {
            let res =  await reqDelMartyrCemetery({id})
            if(res.code!=200){
                return this.toast(res.msg)
            }
            this.init()
        ...
        onAddOne(){
            this.$Router.push({ path: '/pages/martyrCemetery/information', query: { type: 2 }})
        }
    },
    onShow() {
        this.init()
