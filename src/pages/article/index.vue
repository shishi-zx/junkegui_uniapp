<template>
  <div class="container">
    <div class="top">
      <uni-search-bar v-model="searchValue" radius="20" placeholder="请输入标题关键字" @input="search" @cancel="cancel"
        cancel-text="取消">
    ...
    <div class="list-box">
      <uni-list class="list">
        <my-link :to="`{ path: '/pages/articleDetail/index', query: { id: ${item.id},type:1 }}`" navType="push"
...
<script>
import { reqAllVolunteerArticle } from '../../api/index'
export default {
  data() {
    return {
      article_list: [],
  ...
  methods: {
    async init() {
      let res = await reqAllVolunteerArticle()
      if (res.code == 200) {
