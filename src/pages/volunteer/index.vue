<template>
    <div class="container">
        ...
        <div class="list-box">
            <uni-list class="list">
                <my-link :to="`{ path: '/pages/volunteer/information', query: { id: ${item.id},type:1 }}`" navType="push"
...
<script>
import { reqAllVolunteer,reqDelVolunteer } from '../../api/index'
export default {
    data() {
        return {
            list: [],
    ...
    methods: {
        ...
        async onDelete(id) {
            let res =  await reqDelVolunteer({id})
            if(res.code!=200){
                return this.toast(res.msg)
            }
            this.init()
        ...
        onAddOne(){
            this.$Router.push({ path: '/pages/volunteer/information', query: { type: 2 }})
        }
    },
    onShow() {
        this.init()
